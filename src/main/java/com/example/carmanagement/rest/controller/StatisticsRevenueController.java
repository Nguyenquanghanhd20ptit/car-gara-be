...
import com.example.carmanagement.service.statistic.CustomerRevenueDetailService;
...
public class StatisticsRevenueController {
    ...
    @PostMapping(value = "/revenue/customer/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "chi tiet thong ke khach hang theo doanh thu")
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "Success",
            content = {@Content(mediaType = "application/json", schema = @Schema(implementation = PageResponse.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request"),
            @ApiResponse(responseCode = "500", description = "Internal server error")})
    public ResponseEntity<String> searchDetail(@PathVariable("id") Integer id,
                                               @RequestBody SearchRequest request) {
        if (ObjectUtils.isEmpty(request)) {
            return ResponseEntity.badRequest().build();
        }
        return customerRevenueDetailService.getDetail(id,request);
    }
}
